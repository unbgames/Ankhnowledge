...
string SDLBase::resourcesPath = resources_dir_path();
...
GameManager::GameManager() {
	SDLBase::resourcesPath = resources_dir_path();
...
void GameManager::render(float cameraX, float cameraY){
	cameraX = 0;
	cameraY = 0;
	currentScene->render(cameraX, cameraY);
	if(renderQuitBox)
		mb->render(cameraX, cameraY);
	fadeScreen->render(cameraX, cameraY);
}
